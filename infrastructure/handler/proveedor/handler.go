package proveedor

import (
	"github.com/google/uuid"
	"github.com/labstack/echo/v4"
	"github.com/wilzygon/ecommerce/domain/proveedor"
	"github.com/wilzygon/ecommerce/infrastructure/handler/response"
	"github.com/wilzygon/ecommerce/model"
)

type handler struct {
	useCase  proveedor.UseCase
	response response.API
}

func newHandler(useCase proveedor.UseCase) handler {
	return handler{useCase: useCase}
}

// Create handles the creation of a model.Proveedor
func (h handler) Create(c echo.Context) error {
	m := model.Proveedor{}

	if err := c.Bind(&m); err != nil {
		return h.response.BindFailed(err)
	}

	if err := h.useCase.Create(&m); err != nil {
		return h.response.Error(c, "useCase.Create()", err)
	}

	return c.JSON(h.response.Created(m))
}

// Update handles the update of a model.Proveedor
func (h handler) Update(c echo.Context) error {
	m := model.Proveedor{}

	if err := c.Bind(&m); err != nil {
		return h.response.BindFailed(err)
	}

	ID, err := uuid.Parse(c.Param("id"))
	if err != nil {
		return h.response.BindFailed(err)
	}
	m.ID = ID

	if err := h.useCase.Update(&m); err != nil {
		return h.response.Error(c, "useCase.Update()", err)
	}

	return c.JSON(h.response.Updated(m))
}

// Delete handles the deleting of a model.Proveedor
func (h handler) Delete(c echo.Context) error {
	ID, err := uuid.Parse(c.Param("id"))
	if err != nil {
		return h.response.BindFailed(err)
	}

	err = h.useCase.Delete(ID)
	if err != nil {
		return h.response.Error(c, "useCase.Delete()", err)
	}

	return c.JSON(h.response.Deleted(nil))
}

func (h handler) GetByID(c echo.Context) error {
	ID, err := uuid.Parse(c.Param("id"))
	if err != nil {
		return h.response.Error(c, "uuid.Parse()", err)
	}

	proveedorData, err := h.useCase.GetByID(ID)
	if err != nil {
		return h.response.Error(c, "useCase.GetWhere()", err)
	}

	return c.JSON(h.response.Ok(proveedorData))
}

// GetAll handles the search of all model.Product
func (h handler) GetAll(c echo.Context) error {
	proveedores, err := h.useCase.GetAll()
	if err != nil {
		return h.response.Error(c, "useCase.GetAllWhere()", err)
	}

	return c.JSON(h.response.Ok(proveedores))
}

//Tarea
//Paginar el GetAll
//En los parametros, el QueryParam del endPoint van a recibir:
//limit, cuanto registros quiere recibir
//page, en que pagina se quiere mostrar
//Se busca estos datos, se convierte el limit a limit y
//el outset se genera de el limite * cantidad de paginas - el limite
